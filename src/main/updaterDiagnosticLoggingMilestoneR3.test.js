...
//      6.0.0.exe" / ".exe.sha256" assets, the canonical dotted convention)
//      still succeeds end-to-end, confirming naming/tag format was never
//      the actual defect;
...
test('downloadUpdate succeeds end-to-end for the exact reported 5.8.4 -> 6.0.0 upgrade path, and logs each boundary', () => withTempDir(async (userDataPath) => {
  ...
  // canonical dotted artifactName template output) matches cleanly
  // end-to-end.
