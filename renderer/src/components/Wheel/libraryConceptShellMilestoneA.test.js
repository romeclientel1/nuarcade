...
test("the compact selected-game strip sits below the shelf and does not cover the collection row (D4: shelf moved up into Continue Playing's reclaimed space)", () => {
  ...
  assert.match(css, /\.stage \.infoPanel\s*\{[^}]*top:\s*clamp\(456px,\s*45.5vh,\s*488px\)[^}]*width:\s*clamp\(700px,\s*60vw,\s*1150px\)[^}]*min-height:\s*40px/s)
  assert.match(css, /@media \(max-width: 1280px\), \(max-height: 870px\)[\s\S]*?\.stage \.wheelArea\s*\{[^}]*top:\s*150px[^}]*height:\s*210px[\s\S]*?\.stage \.infoPanel\s*\{[^}]*top:\s*366px/s)
})
